...
세 함수 모두 순위를 매기지만 **동점 처리**가 다르다. 예를 들어 값이 `10, 8, 8, 5`일 때:
...
`ROW_NUMBER()`는 동점이어도 무조건 다른 번호, `RANK()`는 동점 시 같은 순위 후 건너뜀, `DENSE_RANK()`는 동점 시 같은 순위 후 건너뛰지 않음.
- **핵심 원리**: 동점 허용 여부와 순위 건너뛰기 여부로 구분
